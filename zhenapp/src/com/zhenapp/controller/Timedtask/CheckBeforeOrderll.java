...
import com.zhenapp.controller.service.Timedtask.CheckBeforeOrderServicell;
import com.zhenapp.po.Custom.TTaskInfoCustom;
import com.zhenapp.service.TaskInfoService;
@Controller
public class CheckBeforeOrderll {
	SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
	SimpleDateFormat yyyyMMdd = new SimpleDateFormat("yyyyMMdd");
	SimpleDateFormat yyyy_MM_dd = new SimpleDateFormat("yyyy-MM-dd");
	private static Logger logger = Logger.getLogger(CheckBeforeOrderll.class);
	@Autowired
	private CheckBeforeOrderServicell checkBeforeOrderServicell;
	@Autowired
	private TaskInfoService taskInfoService;
	/*
	 * 每天0点2分执行一次 将未完成的流量任务自动终止
	 */
	@RequestMapping(value="/api/platform/CheckBeforeOrderll")
	public @ResponseBody ModelMap checkBeforeOrder() throws Exception{
		...
		if(tTaskInfoCustomlistfinish!=null && tTaskInfoCustomlistfinish.size()>0){
			for (int i = 0; i < tTaskInfoCustomlistfinish.size(); i++) {
				...
				map = checkBeforeOrderServicell.updateTaskstateByTime(tTaskInfoCustom);
			}
			logger.info("每天0点2分执行一次 将未完成的任务自动终止");
		}
		return map;
	}
}

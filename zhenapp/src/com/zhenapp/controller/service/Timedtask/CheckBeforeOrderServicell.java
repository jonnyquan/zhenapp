...
@Transactional
@Service
public class CheckBeforeOrderServicell {
		...
		if(endcounts>0){
			...
			if(tUsertestInfoCustomlist!=null && tUsertestInfoCustomlist.size()>0){
				if(tUsertestInfoCustomlist.get(0).getUserroleid().equals("2")
			        ...
			        if(statusCode == 200) {
			            ...
			            if(result.indexOf("delay") != -1){
				            map.put("msg", result);
				            logger.info("凌晨处理未完成任务，自动终止任务成功");
			            }else{
			            	logger.error("凌晨处理未完成任务，自动终止任务失败，失败返回值："+result);
				            throw new RuntimeException();
			            }
			        }else {
			            logger.error("凌晨处理未完成任务，失败错误码："+statusCode);
			            throw new RuntimeException();
			        }
				}
			}else{
		        ...
		        if(statusCode == 200) {
		            ...
		            logger.error("凌晨处理未完成任务，自动终止任务失败，失败错误码："+statusCode);
		            throw new RuntimeException();
		        }
			}
		}

...
public class DownLoadScriptFile {
	...
	public void downloadFile(String name, HttpServletResponse response,HttpServletRequest request)  {
		...
		try {
	        ...
	        bis.close();  
	        bos.close();  
	        logger.info("下载脚本文件完成====================="+name);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
			logger.info("下载脚本文件异常,文件未找到!====================="+name);
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("下载脚本文件异常!====================="+name);
		}
	}
	 ...
	 * http://120.24.44.130/api/phone/down?name=hqfh2.lua
	 */
	@RequestMapping(value = "/api/phone/down/{filename}")
	public @ResponseBody String downloadFilerest(@PathVariable(value="filename")String name, HttpServletResponse response,HttpServletRequest request)  {
		logger.info("开始下载脚本文件");
		response.setCharacterEncoding("utf-8");
		response.setContentType("multipart/form-data");
		try {
